...
synthesises a structured build spec, builds it with BuildKit, climbs a
...
src/
  ...
  builder.py            <- seam: Builder protocol + LocalBuildKitBuilder
...
```bash
uv run scripts/test_envbuild.py       # 46 checks: renderer, every action, rules, goldens
...
Driving from the host needs two things the agent container already has:

```bash
# 1. buildctl, from the SAME pinned builder the daemon runs
docker run --rm --entrypoint cat moby/buildkit:v0.24.0 /usr/bin/buildctl > ~/.local/bin/buildctl
chmod +x ~/.local/bin/buildctl

# 2. a reachable buildkitd -- the base compose file leaves it unpublished on
#    purpose, so add the loopback overlay and point the driver at it
...
ships `LocalBuildKitBuilder` and `LocalDockerVerifier`; the in-cluster versions
swap in without `driver.py` changing.
