...
for required in SKILL.md config.ini compose.yaml specs src/driver.py \
...
rm -rf "$dest/SKILL.md" "$dest/config.ini" "$dest/compose.yaml" \
       "$dest/run.sh" \
...
cp "$src/SKILL.md" "$src/config.ini" "$src/compose.yaml" "$dest/"
